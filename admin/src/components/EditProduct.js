...
const EditProduct = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const product = await getProductById("product", idProduct);
        ...
        const categoryData = await getAllCategories("category");
        setCategoryAll(categoryData.data);
        const tagData = await getAllCategories("tag");
    ...
    const fetchImages = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5019/api/gallary/product/${idProduct}`
        );
  ...
  const handleUploadImages = async (id) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:5019/api/gallary/upload/${id}`,
        formData,
        {
          headers: {
